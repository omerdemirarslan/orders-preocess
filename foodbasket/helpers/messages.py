""" This File Contains Success, Warning, Error and General Messages For API Request-Response Loop and Other Views """

HOME_PAGE = "Welcome To Foodbasket Project. If you want test this django REST API: " \
            "Use the fallowing API request options.\n\nhttp://localhost:8000/api/v1/partners/products/get-products/" \
            "\nhttp://localhost:8000/api/v1/partners/categories/get-categories/ \n" \
            "http://localhost:8000/api/v1/partners/restaurants/get-restaurants/ \n" \
            "http://localhost:8000/api/v1/orders/create-order/ \n" \
            "http://localhost:8000/api/v1/orders/get-order/ \n\n" \
            "THANK YOU FOR FREE TIME"
